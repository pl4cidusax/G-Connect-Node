...
/*JSON*/
    sensor_data["wind"] = _wind_speed;//float
    sensor_data["hum"] = _humidity;//float
    sensor_data["temp"] = _temperature;//float
    sensor_data["soil"] = _soil_moisture;//float
    sensor_data["rain"] = rain_level;//float
    sensor_data["lat"] = latitude;//double
    sensor_data["lon"] = longitude;//double
    sensor_data["roll"] = imu_roll;//float
    sensor_data["pitch"] = imu_pitch;//float
    sensor_data["bat"] = battery_status;//uint8_t
    sensor_data["status"] = danger_status;//uint8_t // danger calculation from nodes
    serializeJson(sensor_data, Serial);
    Serial.println();
